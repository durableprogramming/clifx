...
fn calculate_2d_shine_intensity(pos: &Position2D, shine_line: f32, angle: f32, width: f32, blur: bool) -> f32 {
    ...
    let distance = if angle.abs() < 0.01 {
        ...
        let line_point_distance = (cos_angle * pos.x as f32 + sin_angle * pos.y as f32 - shine_line).abs();
...
pub fn apply_shine2d_effect(text: &str, config: &Shine2DConfig) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let terminal_width = config.terminal_width.unwrap_or_else(|| {
    ...
    let cycles_to_run = if config.cycles == 0 { usize::MAX } else { config.cycles as usize };
    ...
    for cycle in 0..cycles_to_run {
        ...
        for frame in 0..total_frames {
            ...
            let shine_position = match config.start {
                ShineStart::Beginning => back_and_forth_progress * shine_range - config.padding as f32,
                ShineStart::End => (1.0 - back_and_forth_progress) * shine_range - config.padding as f32,
            };
