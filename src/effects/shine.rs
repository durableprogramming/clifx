...
pub fn apply_shine_effect(text: &str, config: &ShineConfig) -> Result<(), Box<dyn std::error::Error>> {
    ...
    execute!(stdout, terminal::Clear(ClearType::CurrentLine), cursor::Hide)?;
    ...
    for cycle in 0..cycles_to_run {
        ...
        for frame in 0..total_frames {
            ...
            let shine_position = match config.start {
                ShineStart::Beginning => (back_and_forth_progress * (total_range as f32 - 1.0)) as isize - config.padding as isize,
                ShineStart::End => ((1.0 - back_and_forth_progress) * (total_range as f32 - 1.0)) as isize - config.padding as isize,
            };
