...
usage() {
    ...
    echo ""
    echo "Examples:"
    echo "  $0 10      # Test on Debian 10 (Buster)"
    echo "  $0 11      # Test on Debian 11 (Bullseye)"
    echo "  $0 12      # Test on Debian 12 (Bookworm)"
    echo ""
    echo "This script will:"
    echo "  1. Build a .deb package if not already present"
    echo "  2. Create a Docker container with the specified Debian version"
...
create_dockerfile() {
    ...
    local dockerfile_content="FROM debian:$debian_version
...
test_in_docker() {
    ...
    log_info "Creating Docker test environment for Debian $debian_version"
    
    # Copy deb to temp directory
    cp "$deb_path" "$temp_dir/"
    
    # Create Dockerfile
    create_dockerfile "$debian_version" > "$temp_dir/Dockerfile"
    
    # Build Docker image
    local image_name="clifx-test-debian-$debian_version"
    ...
    if docker run --rm "$image_name"; then
        echo "----------------------------------------"
        log_success "All tests passed on Debian $debian_version!"
    else
        echo "----------------------------------------"
        log_error "Tests failed on Debian $debian_version"
        exit 1
    fi
...
main() {
    ...
    if ! [[ "$debian_version" =~ ^[0-9]+$ ]]; then
        log_error "Invalid Debian version format. Expected: single number (e.g., 10, 11, 12)"
        exit 1
    fi
    
    log_info "Starting Debian $debian_version integration test"
    ...
    # Test in Docker
    test_in_docker "$debian_version" "$deb_path"
